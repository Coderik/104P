...
{
public:
	static Glib::RefPtr<Gdk::Pixbuf> get_magnitudes_view(const ImageFx<float> &flow);
	static Glib::RefPtr<Gdk::Pixbuf> get_color_code_view(const ImageFx<float> &flow);

private:
	static float get_max_length(const ImageFx<float> &flow);
	static Glib::RefPtr<Gdk::Pixbuf> get_view(const ImageFx<float> &flow, IOptivalFlowCode *optical_flow_code);
